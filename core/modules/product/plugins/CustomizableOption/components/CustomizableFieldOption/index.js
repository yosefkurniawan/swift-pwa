...
import View from '@plugin_customizableitem/components/CustomizableFieldOption/view';
...
const CustomizableFieldOption = ({
    ...
    const { data, loading } = useQuery(getCustomizableFieldOption(url_key), {
        skip: !url_key,
    ...
    useMemo(() => {
        if (data && data.products && data.products.items.length > 0) {
            const option = data.products.items[0].options.filter(
                (item) => item.option_id === option_id && item.__typename === 'CustomizableFieldOption',
            );
...
export default CustomizableFieldOption;
